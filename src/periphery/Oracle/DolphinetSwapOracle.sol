...
import "./DolphinetSwapOracleStorage.sol";

/// @title DolphinetSwapOracle
/// @notice A TWAP oracle based on DolphinetSwap Pair (time-weighted average price)
contract DolphinetSwapOracle is DolphinetSwapOracleStorage {
    ...
    constructor(address _pair, uint256 _minTwapPeriod) {
        ...
        pair = IDolphinetSwapPair(_pair);
        MIN_TWAP_PERIOD = _minTwapPeriod;

        address _token0 = pair.token0();
        address _token1 = pair.token1();
        if (_token0 == address(0) || _token1 == address(0)) {
            revert DolphinetSwapOracle_InvalidPair();
        }
        token0 = _token0;
        token1 = _token1;
    ...
    function update() external {
        ...
        if (timeElapsed < MIN_TWAP_PERIOD) {
            revert DolphinetSwapOracle_InsufficientElapsedTime();
        }
    ...
    function consult(address tokenIn, uint256 amountIn) external view returns (uint256 amountOut) {
        ...
        if (tokenIn == token0) {
            ...
            revert DolphinetSwapOracle_InvalidToken();
        }
    }
