...
import "../interfaces/IDolphinetSwapRouter02.sol";
import "../interfaces/IDolphinetSwapPair.sol";
import "../libraries/DolphinetSwapLibrary.sol";
import "../interfaces/IWETH.sol";

contract DolphinetSwapRouter02 is IDolphinetSwapRouter02 {
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = DolphinetSwapLibrary.sortTokens(input, output);
            IDolphinetSwapPair pair = IDolphinetSwapPair(DolphinetSwapLibrary.pairFor(factory, input, output));
            uint256 amountInput;
            uint256 amountOutput;
            {
                ...
                amountOutput = DolphinetSwapLibrary.getAmountOut(amountInput, reserveInput, reserveOutput);
            }
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(uint256 amountIn, uint256 amountOutMin, address[] calldata path, address to, uint256 deadline) external virtual override ensure(deadline) {
        _safeTransferFrom(path[0], msg.sender, DolphinetSwapLibrary.pairFor(factory, path[0], path[1]), amountIn);
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        uint256 balanceAfter = IERC20(path[path.length - 1]).balanceOf(to);
        require(balanceAfter - balanceBefore >= amountOutMin, "DolphinetSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    }
    ...
    function swapExactETHForTokensSupportingFeeOnTransferTokens(uint256 amountOutMin, address[] calldata path, address to, uint256 deadline) external payable virtual override ensure(deadline) {
        ...
        require(IWETH(WETH).transfer(DolphinetSwapLibrary.pairFor(factory, path[0], path[1]), amountIn), "DolphinetSwapRouter: WETH_TRANSFER_FAILED");
        uint256 balanceBefore = IERC20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        uint256 balanceAfter = IERC20(path[path.length - 1]).balanceOf(to);
        require(balanceAfter - balanceBefore >= amountOutMin, "DolphinetSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT");
    ...
    function swapExactTokensForETHSupportingFeeOnTransferTokens(uint256 amountIn, uint256 amountOutMin, address[] calldata path, address to, uint256 deadline) external virtual override ensure(deadline) {
        ...
        require(amountOut >= amountOutMin, "DolphinetSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT");
