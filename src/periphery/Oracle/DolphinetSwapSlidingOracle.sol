...
import "./DolphinetSwapSlidingOracleStorage.sol";

/// @title DolphinetSwap Sliding Window TWAP Oracle
/// @notice Computes TWAP using multiple historical observations (sliding window)
contract DolphinetSwapSlidingOracle is DolphinetSwapSlidingOracleStorage {
    ...
    function currentCumulativePrices(address pair) public view returns (uint256 price0Cumulative, uint256 price1Cumulative, uint32 blockTimestamp) {
        IDolphinetSwapPair DolphinetSwapPair = IDolphinetSwapPair(pair);
        price0Cumulative = DolphinetSwapPair.price0CumulativeLast();
        price1Cumulative = DolphinetSwapPair.price1CumulativeLast();

        (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast_) = DolphinetSwapPair.getReserves();
    ...
    function consult(address tokenIn, uint256 amountIn, address tokenOut) external view returns (uint256 amountOut) {
        address pair = DolphinetSwapLibrary.pairFor(factory, tokenIn, tokenOut);
        ...
        (address token0, ) = DolphinetSwapLibrary.sortTokens(tokenIn, tokenOut);
